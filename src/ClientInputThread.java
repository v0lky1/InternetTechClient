...
//        client.retrieveFile("received.jpg", client.getFileContents("test.jpg"));
        ...
        while (!client.isStopped()) {
            ...
            if (message.startsWith("/")) {
                ...
                // This merges the command (which is now CAPITALIZED) and the rest of the message back together.
                message = splitMessage.length == 1 ? userCommand : userCommand + " " + splitMessage[1];

                if (userCommand.equals("SENDFILE")) {
                    validCommand = true;
                    // Call sendfile
                    splitMessage = splitMessage[1].split(" ", 2);
                    String file = Base64.getEncoder().encodeToString(client.getFileContents(splitMessage[1]));
